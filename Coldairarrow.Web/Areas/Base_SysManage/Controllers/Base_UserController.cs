...
namespace Coldairarrow.Web
{
    public class Base_UserController : BaseMvcController
    {
        ...
        public ActionResult SaveData(Base_User theData, string Pwd, string RoleIdList,string DepartmentIdList)
        {
            if (!Pwd.IsNullOrEmpty())
                theData.Password = Pwd.ToMD5String();
            var roleIdList = RoleIdList.ToList<string>();
            var departmentIdList = DepartmentIdList.ToList<string>();
            ...
            _base_UserBusiness.SetUserDepartment(theData.UserId, departmentIdList);
